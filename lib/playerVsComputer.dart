...
class _playerVsComputerState extends State<playerVsComputer> {
  ...
  var computerMove = [0, 1, 2, 3, 4, 5, 6, 7, 8];
  ...
  int computerChoice(bool Xturn) {
    if (Xturn == true) {
      var comIndex = computerMove[_random.nextInt(computerMove.length)];
      setState(() {
        changeXO[comIndex] = "O";
        computerMove.remove(comIndex);
        filledbox += 1;
        print("$filledbox items filled");
        Xturn = !Xturn;
      });
    }
  }
  ...
  change(int index) {
    setState(() {
      if (Xturn == true && changeXO[index] == "") {
        changeXO[index] = "X";
        // Xturn = !Xturn;
        computerMove.remove(index);

        filledbox += 1;
        print("$filledbox filled persion");
        computerChoice(Xturn);
        // Xturn = !Xturn;
      }
      // else if (!Xturn && changeXO[index] == "") {
      //   var comIndex = computerChoice();
      //   changeXO[comIndex] = "O";
      //   computerMove.remove(comIndex);
    ...
    if (changeXO[0] == changeXO[1] &&
      ...
      if (changeXO[0] == "X") {
        int i = await DatabaseHelper.instance.insert({
          DatabaseHelper.columnName: "Player VS Computer",
          DatabaseHelper.gameStatus: "Player wins",
          DatabaseHelper.statusColor: "Green"
        });
        print(i);
      } else {
        int i = await DatabaseHelper.instance.insert({
          DatabaseHelper.columnName: "Player VS Computer",
          DatabaseHelper.gameStatus: "Computer wins",
          DatabaseHelper.statusColor: "Red"
        });
        print(i);
      }
      ...
      int i = await DatabaseHelper.instance.insert({
        ...
        DatabaseHelper.statusColor: "Grey"
      });
      print(i);
      _interstitialAd.show();
      _showDrawDialog();
  ...
  void _clearBoard() {
    for (var i = 0; i < 9; i++) {
      setState(() {
        ...
        filledbox = 0;
        Xturn = true;
        computerMove = [0, 1, 2, 3, 4, 5, 6, 7, 8];
      });
    }
  }
